...
tanks_images = [pygame.image.load(config.images_dir+f'\\tanks\\{img}') for img in os.listdir(
    config.images_dir+f'\\tanks')]  # [pygame.image.load(f'images/tanks/tank_{i}.png').convert() for i in range(1, os.listdir())]

class Tank:
    tanks = []
    id_tank = 0
    def __init__(self, screen, name, keys=None, cords=(0, 0), size=4, speed=10, image=tanks_images[0]):
    ...
    def get_data(self):
        ...
        return [self.distance/120, self.side_obj/38]
    ...
    def update(self, keys):
        ...
        if keys[self.keySHOT] and self.flag_bullet_spawn:
            self.gun()
            self.flag_bullet_spawn = False
...
class TankNeiro(Tank):
    def __init__(self, screen, name, cords=(0, 0), size=4, image=tanks_images[0]):
